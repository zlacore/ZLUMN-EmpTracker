...
function loadDepts() {
    ...
    //RETURN depts array
    pool.query('SELECT name FROM department', (err, result) => {
        if (err) {
            console.log(err);
        }
        else {
...
function loadRoles() {
    ...
    //PUSH department names into depts array
    //RETURN depts array
    pool.query('SELECT title, id FROM role', (err, result) => {
        if (err) {
            console.log(err);
        }
        else {
...
function loadEmps() {
    ...
    pool.query('SELECT first_name, last_name FROM employee', (err, result) => {
        if (err) {
            console.log(err);
        }
        else {
...
inquirer
    ...
    if (answers.actions === "View departments") {
        ...
        pool.query('SELECT * FROM department ORDER BY id ASC', (err, result) => {
            if (err) {
                console.log(err);
            }
            else {
        ...
        inquirer
            ...
            pool.query('INSERT INTO department(name) VALUES($1) RETURNING *', [deptans.deptinput], (err) => {
                if (err) {
                    console.log(err);
                }
                else {
        ...
        inquirer
            ...
            deptToID()
                ...
                pool.query('INSERT INTO role (title, salary, department) VALUES($1, $2, $3) RETURNING *', [roleans.rolename, roleans.rolesalary, deptID], (err) => {
                    if (err) {
                        console.log(err);
                    }
                    else {
        ...
        inquirer
            ...
            // roleToID()
            //     .then(roleID => {
            pool.query('INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES($1, $2, $3, $4)', [empAns.empFirstName, empAns.empLastName, empAns.empRole, empAns.empMgr], (err) => {
                if (err) {
                    console.log(err);
                }
                else {
                    ...
                    let newEmp = {
                        firstName: empAns.empFirstName,
                        lastName: empAns.empLastName
                    };
                    employees.push(newEmp);
                }
            });
        ...
        inquirer
            ...
            // ////////////////////////////////////////////////////////////////////////////////////////////
            pool.query('UPDATE employee SET role_id = $1 WHERE employee = $2', [updRole.value, updEmp], (err) => {
                if (err) {
                    console.log(err);
                }
                else {
                    console.log(`${updEmp}'s role has been updated to ${updRole}`);
                }
            });
        });
    }
